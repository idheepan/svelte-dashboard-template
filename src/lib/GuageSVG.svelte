<script lang="ts">
	...
	function describeArc(x: number, y: number, startAngle: number, endAngle: number) {
	...
	function createArcSection(start_angle: number, end_angle: number, color: string, svg: Element) {
		var arc = document.createElementNS(svgNS, 'path'); //to create a circle. for rectangle use "rectangle"
		arc.setAttribute('d', describeArc(center.x, center.y, start_angle, end_angle));
