...
<script lang="ts">
  ...
  // TODO : Calculate the energy removed from air from a start time to now.
  // What is the enthalpy of incoming air right now?
  // What is the enthalpy of outgoing air right now?
  // Over a period from start of test to now, what was the total enthalpy of INLET air?
  // Over a period from start of test to now, what was the total enthalpy of **OUTLET** air?
  // What is the difference of the above two values right now?
    ...
    await fetch(request_string, { method: "GET" })
      ...
      .then((data) => {
        if (data.length > 0) {
          ...
          if (data[data.length - 1].sensor === 1) {
